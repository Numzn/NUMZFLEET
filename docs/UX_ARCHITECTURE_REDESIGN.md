...
**Revised** from the original "no shadow" call below, after shipping it: light mode had `--surface-app`, `--surface-card`, and `--surface-elevated` all collapsed to `#FFFFFF` with `boxShadow: none`, so a card had *zero* depth cue of any kind — not a border-only aesthetic, an absent one. Dark mode never had this problem; it already carries four visibly distinct tones (app < workspace < card < elevated). Fixed both: light mode now gets the same kind of tonal separation (`--surface-app: #F7F8FA`, `--surface-card: #FFFFFF`), and cards carry `--shadow-subtle` (previously defined, unused) as a light lift on top of that — not instead of it.

```
Level 0  page background     var(--surface-app)      — tinted, not white
Level 1  card                surface-card + 1px border + shadow-subtle
Level 2  popover / dropdown  surface-card + border + shadow-elevation
Level 3  modal / drawer      surface-card + shadow-elevation + scrim
```

`--shadow-elevation` stays reserved for things that actually float — modals, popovers, drag states, drawers — that distinction is unchanged. What changed is that Level 1 is no longer bare border on an undifferentiated background; it needed a real surface to sit on before "no shadow" could read as intentional flatness rather than missing chrome.
