...
export function startTelemetryReconciliationScheduler() {
  ...
  if (!Number.isFinite(intervalMs) || intervalMs <= 0) {
    if (process.env.NODE_ENV === 'development') {
      console.log('[telemetryReconciliation] disabled (TELEMETRY_RECONCILE_INTERVAL_MS is 0 or invalid)');
    }
    return () => {};
  }

  const startupDelayMs = Math.max(0, parseInt(process.env.TELEMETRY_RECONCILE_STARTUP_DELAY_MS ?? '30000', 10) || 0);
  ...
  return runIntervalJob({
    name: 'telemetryReconciliation',
    intervalMs,
    startupDelayMs,
    lockKey: LOCK_KEYS.TELEMETRY_RECONCILIATION,
    task,
  });
}
